...
struct HgetrfConfig {
    // 是否使用Tensor Core加速GEMM
    bool use_tensor_core_gemm = true;

    // TRSM实现方式
    enum TrsmMode {
        TRSM_CUSTOM_KERNEL,   // 自定义kernel
        TRSM_CUBLAS_HALF,     // cuBLAS half精度
        TRSM_CUBLAS_FLOAT     // cuBLAS float精度
    };
    ...
    HgetrfTimers* timers)
{
    if (!dA || !d_ipiv_rel || !h_ipiv_rel || !piv_rows) {
        fprintf(stderr, "hgetrf_dualstream: null pointer!\n");
        std::exit(EXIT_FAILURE);
    }
    ...
    if (config.verbose) {
        printf("[hgetrf] 双流模式, panel宽度: %d, panel数: %d\n",
...
{
    if (!dA || !d_ipiv_rel || !h_ipiv_rel || !piv_rows) {
        fprintf(stderr, "hgetrf_auto: null pointer!\n");
        std::exit(EXIT_FAILURE);
    }
    ...
    // 如果矩阵很小（只有一个panel），直接分解
    if (n <= panel_width) {
        if (config.verbose) {
            printf("[hgetrf_auto] 单panel模式 (n=%d)\n", n);
        }
    ...
    // 多panel情况：使用双流流水线
    if (config.verbose) {
        printf("[hgetrf_auto] 双流模式 (n=%d, panel宽度=%d)\n", n, panel_width);
    }
